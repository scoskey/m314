...
\begin{definition}
  Let $A$ be a subset of $\R$. A point $x$ is said to be a \emph{boundary point} of $A$ if every neighborhood $N_\epsilon(x)$ meets both $A$ and $A^c$. The set of all boundary points of $A$ is denoted $\partial A$.
\end{definition}
...
We next investigate the relationship between the boolean operations of union and intersection, and the concepts of open and closed sets. In order to proceed, we need to establish a characterization of open sets in terms of the interior of the set.
...
\begin{proof}
  For the first statement let $\mathcal A$ be a collection of open sets, and $S=\bigcup\mathcal A$. By the previous proposition, given any $x\in S$ we must show that $x$ is an interior point of $S$. Let $A\in\mathcal A$ be such that $x\in A$. Since $x$ is an interior point of $A$, there exists $\epsilon$ such that $N_\epsilon(x)\subset A$. Hence also $N_\epsilon(x)\subset S$, so $x$ is an interior point of $S$.

  For the second statement let $A_1,\ldots,A_n$ be open sets, let $T=A_1\cap\cdots\cap A_n$, and let $x\in T$. Then for all $i\leq n$ there exists $\epsilon_i$ such that $N_{\epsilon_i}(x)\subset A_i$. Letting $\epsilon=\min(\epsilon_1,\ldots,\epsilon_n)$ we clearly have that $N_\epsilon(x)\subset T$. Thus $x$ is an interior point of $T$, as desired.
\end{proof}
...
\begin{theorem}[Calculus of continuity]
...
\begin{proof}
  Let $a$ be an arbitrary point of $f^{-1}(V)$. Since $V$ is open, $f(a)$ has a neighborhood $N_\epsilon(f(a))$ which is contained in $V$. Since $f$ is continuous at $a$ we may let $\delta$ be the corresponding value from the definition of continuity. Now we claim that $N_\delta(a)$ is contained in $f^{-1}(V)$. Indeed, for any $x\in N_\delta(a)$ we have $|x-a|<\delta$ and hence $|f(x)-f(a)|<\epsilon$. This means that $f(x)\in V$ or in other words $x\in f^{-1}(V)$.
\end{proof}
