...
\begin{proof}
  ...
  Now since $a_n$ is bounded, it follows from the Bolzano--Weierstra\ss\ theorem that $a_n$ has a convergent subsequence $a_{n_k}$ with limit $L$. We claim that the whole sequence $a_n$ converges to $L$ as well. Indeed given $\epsilon$ we can choose $N$ large enough that $m,n\geqN$ implies $|a_n-a_m|<\epsilon/2$. Moreover we can find $k$ large enough that $n_k\geq N$ and $|a_{n_k}-L|<\epsilon/2$. Therefore
  \begin{align*}
