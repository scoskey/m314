...
\begin{defn}
  A cut $C$ is said to be an \emph{irrational cut} or a \emph{gap} if the complement of the cut, $\QQ\smallsetminus C$, has no least element.
\end{defn}
...
\begin{exerc}[see Abbott, ex 8.6.1]
  ...
  \begin{enumerate}
  \item $\set{t\in\QQ\mid t<r}$, where $r$ is a rational number
  \item $\set{t\in\QQ\mid t\leq2}$
  \item $\set{t\in\QQ\mid t^2<2}$
  \item $\set{t\in\QQ\mid t^2<2\text{ or }t<0}$
  \item $\set{t\in\QQ\mid t^2\leq2\text{ or }t<0}$
  \end{enumerate}
\end{exerc}

\begin{exerc}[Abbott, ex 8.6.2]
  Suppose that $C$ is a cut, $q\in C$, and $r\notin C$. Show that $q<r$.
\end{exerc}

\begin{exerc}[Abbott, ex 1.2.1]
  Study the proof that there is no rational number $r$ such that $r^2=2$.
  \begin{enumerate}
  \item Modify this proof to show that there is no rational number $r$ such that $r^2=3$.
  \item Obviously there \emph{is} a rational number such that $r^2=4$, so your proof in part (a) cannot work if $3$ is replaced by $4$. But where exactly does your proof break down in this case?
  \end{enumerate}
\end{exerc}

\begin{exerc}
  Let $P$ and $Q$ be points in the plane.
  \begin{enumerate}
  \item Suppose that $P$ is $3$ units away from the origin and $Q$ is $5$ units away from the origin.  What is the farthest $P$ and $Q$ can be from each other?  The closest?
  \item Now generalize.  Suppose that $P$ is $|P|$ units away from the origin and that $Q$ is $|Q|$ units away from the origin.  Use the notation $|P-Q|$ for the distance between $P$ and $Q$.  Can you write two rules that generalize your two answers from part~(a)? (Explain your answer with a picture.  You do not need to give a proof.)
  \end{enumerate}
\end{exerc}
...
\begin{exerc}[Abbott, ex 8.6.3]
  Read the properties of a ``field'' of numbers.  Which of these properties is satisfied by $\NN$?  By $\ZZ$?  By $\QQ$?
\end{exerc}

\begin{exerc}
  Suppose that $A$ and $B$ are subsets of $\RR$ and that $A\subset B$.  Which of the following is right, and why? ``any upper bound for $A$ is also an upper bound for $B$'', or; ``any upper bound of $B$ is also an upper bound for $A$.''
