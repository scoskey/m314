...
\begin{activity}
  ...
  \begin{enumerate}
    \item $\sum\frac{1}{\ln(n)}$\vspace\fill
...
\begin{activity}
  \item (Abbott, exercise 2.5.3) Give an example of each of the following, or prove that no such example exists.
  \begin{enumerate}\itemsep\fill
...
\begin{activity}
  ...
  \begin{enumerate}\itemsep\fill
    \item $\displaystyle\sum\frac{1}{n^2+n+2}$
    \item $\displaystyle\sum\frac{1}{n^2-n+2}$
    \item $\displaystyle\sum\frac{1}{3n-2}$
    \item $\displaystyle\sum\frac{1}{3n+2}$
    \item $\displaystyle\sum\frac{(n+1)^{2n}}{(2n^2+n+1)^n}$
...
\begin{activity}
  \item Decide whether the given series converge absolutely, converge conditionally, or diverge.
  \begin{enumerate}\itemsep\fill
...
\begin{activity}
  \item For each of the following sets $A$, find the boundary $\partial A$. Then decide whether the set is open, closed, both, or neither.
  \begin{enumerate}\itemsep\fill
...
% show that closure A = A union dA is closed
...
\begin{activity}
  ...
  \begin{enumerate}\itemsep\fill
    ...
    \item $A=\set{x\mid (\exists n\in\N)\,\frac1{2n+1}\leq x\leq\frac1{2n}}$
    \vspace\fill
  \end{enumerate}
\end{activity}
...
% Suggestion for streamlined definition of connected.

% Motivation: Consider the ways intervals can abut.
% [0,1]u[1,2]. The sets overlap. The result is connected.
% [0,1]u(1,2]. The sets are disjoint. But the boundaries meet. Even stronger, the boundary of one set meets the other set. The result is connected.
% [0,1)u(1,2]. The boundaries meet. But at least the boundary of each set doesn't meet the other set. The result is disconnected.

% A is connected if for any partition of A into nonempty sets BuC, either dB meets C or dC meets B.
% A is disconnected if you can partition it into nonempty sets BuC, such that dB,C are disjoint and B,dC are disjoint.
...
\begin{activity}
  ...
  \begin{enumerate}\itemsep\fill
    ...
    \item $A=$ any countable infinite subset of $\R$
    \vspace\fill
  \end{enumerate}
...
\begin{definition}
  ...
  \item $x_n$ converges to $x$ if $(\forall\epsilon>0)(\exists N\in\N)(\forall n>N)\,d(x_n,x)<\epsilon$
  \item $x_n$ is Cauchy if $(\forall\epsilon>0)(\exists N\in\N)(\forall m,n>N)\,d(x_m,x_n)<\epsilon$
...
% It is easy to fill in the omitted definitions of interior, exterior, and several others.

Since we have defined convergent and Cauchy sequences, it is natural to ask whether the Cauchy criterion is true in metric spaces. The answer is not in general! For example, if $X$ is the space $(0,1)$, then the sequence $1/n$ is a Cauchy sequence in $X$ with no limit in $X$. This means we need a name for the situation when the Cauchy criterion is true in $X$.

\begin{definition}
  A metric space $X$ is \emph{complete} if every Cauchy sequence is convergent.
\end{definition}

In this terminology, $\R$ is complete (by the actual Cauchy criterion), and the spaces $\R^n$ are complete too. On the other hand, the space $X=(0,1)$ is not complete.
...
When $f_n\to f$ in the uniform metric, we say that the sequence $f_n$ \emph{converges uniformly} to $f$. In the future we will address the missing component in the above proof by showing that whenever a sequence of continuous functions converges uniformly to a limit, the limit must be continuous.

\newpage
\subsection*{Activity for \S \thesection}

\begin{activity}
  \item For each of the distance functions below, verify that it is a metric on $\R^2$. Then plot the set of points in the plane that are exactly one unit away from the origin.
  \begin{enumerate}
    \item $\displaystyle d(\mathbf{x},\mathbf{y})=\max\set{|x_1-y_1|,|x_2-y_2|}$
    \vspace\fill
    \item $\displaystyle d(\mathbf{x},\mathbf{y})=|x_1-y_1|+|x_2-y_2|$
    \vspace\fill
    \item $\displaystyle d(\mathbf{x},\mathbf{y})=\begin{cases}1&x\neq y\\0&x=y\end{cases}$
    \vspace\fill
  \end{enumerate}
  \item Give an informal explanation why the following is a metric on $C[0,1]$. What does it mean for a function $f$ to be in $N_\epsilon(0)$?
  \[d(f,g)=\int_0^1|f-g|
  \]
  \vspace\fill
\end{activity}
...
\begin{activity}
  ...
  \begin{enumerate}
    \item $\displaystyle\lim_{x\to3}x^2=9$
    \vspace\fill
    \item $\displaystyle\lim_{x\to4}\sqrt{x}=2$
    \vspace\fill
  \end{enumerate}
  \item Evaluate the following using the limit laws.
  \begin{enumerate}
    \item $\displaystyle\lim_{x\to0}\frac{x^3+2x^2+3x+1}{x^5+x+2}$
    \vspace\fill
    \item $\displaystyle\lim_{x\to-2}\frac{x^2-x-6}{3x^2+5x-2}$
    \vspace\fill
  \end{enumerate}
\end{activity}
...
\begin{definition}
  Let $f$ be a real-valued function defined on an interval of $\R$. We say $f$ is \emph{continuous} at a point $a$ in its domain if $\lim_{x\to a}f(x)$ exists and equals $f(a)$. We say that $f$ is \emph{continuous} if it is continuous at every point of its domain.
\end{definition}
...
\begin{theorem}[Continuity calculus]
  ...
  \begin{enumerate}
    ...
    \item $f/g$ is continuous, provided $g(x)\neq 0$
...
\begin{example}
  The function $f(x)=\sin(x)$ is continuous. Since the continuity calculus doesn't apply, we must check this directly from the definition. Without going into details, it is not difficult to see geometrically that we always have $|\sin(x)-\sin(a)|\leq|x-a|$, so we may take $\delta=\epsilon$ in the proof.
\end{example}
...
\begin{activity}
  ...
  % \end{enumerate}
  \item Let $f(x)=\frac{3}{1+x^2}$.
  \begin{enumerate}
    \item Let $V=(1,2)$ and find $f^{-1}(V)$ (a graph may help). Is it open?
    \vspace\fill
    \item Let $W=(-1,1)$ and find $f^{-1}(V)$. Is it open?
    \vspace\fill
    \item Let $V=(1,2)$ and find $f(V)$. Is it open?
    \vspace\fill
    \item Let $W=(-1,1)$ and find $f(W)$. Is it open?
    \vspace\fill
  \end{enumerate}
...
\end{itemize}
The forward image mapping $f(\cdot)$ also preserves unions, but fails to preserve intersections or differences. For counterexamples, one can often use the function $f(x)=x^2$.
...
\begin{activity}
  ...
  \begin{enumerate}\itemsep\fill
    ...
    \item A continuous function $f$, whose domain is a bounded interval, and a set $A$ such that $f(A)$ is not bounded.
    \item A continuous function $f$, whose domain is all of $\R$, and a bounded set $A$ such that $f(A)$ is not bounded.
...
The derivative is one of the most important developments of modern mathematics because it captures the intuitive notion of instantaneous rate of change. As simple as a ``rate of change'' may seem, one must be careful with the ``instantaneous'' part since it isn't always well defined. Our study of continuous functions and limits leaves us prepared to explore the concept rigorously.
...
\begin{theorem}[Derivative calculus]
  Asusme $f,g$ are differentiable functions with appropriate domains. Then we have:
  \begin{itemize}
    \item $(f+g)'=f'+g'$
    \item $(kf)'=kf'$
    \item $(fg)'=f'g+fg'$
    \item $(f/g)'=(f'g-g'f)/g^2$
    \item chain rule: $(g\circ f)'=(g'\circ f)f'$
    \item inverse rule: $(f^{-1})'=1/(f'\circ f^{-1})$ if $f$ is invertible
  \end{itemize}
\end{theorem}

We leave the proofs of these theorems to your old calculus textbook.
...
Recall that we have already seen that continuous functions and compact sets are important in optimization because of the Extreme Value Theorem. You surely recall that derivatives are also important in optimization, and that is due to the following result.

\begin{lemma}[Interior Extremum Theorem, or, Critical Point Lemma]
  \label{thm:iet}
...
\begin{activity}
  ...
  \begin{enumerate}
    ...
    \vspace\fill
  \end{enumerate}
  \item Decide, with justification, whether each of the following functions is differentiable at $x=0$.
  \begin{enumerate}
...
\begin{example}
  ...
  \end{align*}
  Here we are using a classic formula for the sum of the first $n$ squares. It is easy to calculate that $\lim U(f,P_n)=b^3/3$, and since $U(f)$ is the infemum over all possible $U(f,P)$, we conclude that $U(f)\leq b^3/3$.
  
  Similarly the minimum falues occur at left endpoints, which means we always have $m_i=(b(i-1)/n)^2$. We thus obtain:
  \begin{equation}
    L(f,P_n)=\frac{b^3}{n^3}\frac{(n-1)n(2n-1)}{6}
  \end{equation}
  Using the same logic as above, it follows that $L(f)\geq b^3/3$, and we conclude from these inequalities that $\int_0^bf\,dx$ exists and equals $b^3/3$.
\end{example}
...
\begin{activity}
  \item Use the definition of integral to calculate the value of the integral:
  \[\int_0^bx^3\,dx
  \]
  Hint: You will need the sum of cubes formula:
...
\begin{proof}
  We can add a linear term to $f$ to assume without loss of generality that $f(a)=f(b)$. (Note that passing from $f(x)$ to $f(x)+mx+b$ has the effect of adding an $m$ to both sides of the desired equation.) This reduces the statement to Rolle's theorem, which uses the Extreme Value Thoerem together with the Interior Extremum Theorem to conclude that there must exist $c\in[a,b]$ such that $f'(c)=0$.
\end{proof}
...
\begin{proof}
  ...
  \[F'(x)=\lim_{h\to0}\frac{1}{h}\int_x^{x+h} f(t)\,dt
  \]
  ...
  \]
  Dividing all sides by $h$ and taking the limit, we have:
...
\begin{activity}
  ...
  \begin{enumerate}
    \item $F(x)=\int_0^x \cos(3t)\,dt$\vspace\fill
    \item $F(x)=\int_0^{\ln(x)}\cos(3t)\,dt$\vspace\fill
    \item $F(x)=\int_0^x \cos(t^2)\,dt$\vspace\fill
    \item $F(x)=\int_0^{\ln(x)} \cos(t^2)\,dt$\vspace\fill
  \end{enumerate}
  \item Suppose you know $f(2)=5$ and $-.5\leq f'(x)\leq.5$ for all $x$. What is the biggest $f(10)$ can be? What is the smallest? How do you prove it?
  \vspace\fill
\end{activity}
...
The example of Thomae's function naturally leads to the question: If we can find a function $f$ such that $D_f$ is the set of rational numbers, can we also find a function $f$ such that $D_f$ is the set of irrational numbers? With all our ingenuity to create bizarre functions, it may surprise you that the answer is ``no''. To help us reach this conclusion, we need the following partial characterization of the sets $D_f$.

\begin{theorem}
  \label{thm:df-closed}
  If $f\colon\R\to\R$, then $D_f$ can be written as a union of countably many closed sets.
\end{theorem}
