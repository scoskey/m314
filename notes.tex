...
For example, the following triple is a cut:
\begin{equation}
  \label{eq:rationalcut}
  \left(\set{q\in\Q\mid q<3/2},\set{3/2},\set{q\in\Q\mid q>3/2}\right)\text{.}
\end{equation}
This type of cut with $P$ containing a single rational number is somewhat obvious to define and is called a \emph{rational cut}.
...
For another example, the following triple is a cut:
\begin{equation}
  \label{eq:irrationalcut}
  \left(\set{q\in\Q\mid q<0\text{ or }q^2<2},\emptyset,\set{q\in\Q\mid q\geq0\text{ and }q^2>2}\right)\text{.}
\end{equation}
This is the cut at $\sqrt{2}$ considered previously, though we leave it until the next section to prove that $A$ has no greatest element and $B$ has no least element. This type of cut with $P=\emptyset$ is called a \emph{gap} or \emph{irrational cut}.
...
We will then describe a correspondence between cuts and real numbers, with rational cuts corresponding to rational numbers and irrational cuts corresponding to irrational numbers.
...
In the previous section we defined a cut in the rational number system, and gave examples of both rational and irrational cuts. It is relatively straightforward to prove that the triple in Equation~\eqref{eq:rationalcut} is really a cut. But we still need to prove that the triple in Equation~\eqref{eq:irrationalcut} (the cut at $\sqrt2$) is really a cut. The proof turns out to be more technical than one might have expected. It will provide us with a not-so-soft example of the kind of arguments we will see later in the course.

\begin{theorem}
  \label{thm:root-2-cut}
  The triple in Equation~\eqref{eq:irrationalcut} is a cut.
\end{theorem}

\begin{proof}
  It is not difficult to verify properties (a) and (b) of a cut. Thus we need only show that the set $A=\set{q\in\Q\mid q<0\text{ or }q^2<2}$ has no greatest element. The proof that $B$ has no least element is nearly identical.
  
  Given any $q\in A$, we will show $q$ is not the greatest element of $A$. It is clear that a negative number cannot be the greatest element of $A$, so let's assume that $q\geq0$ and $q^2<2$. We wish to find a small rational number $\epsilon>0$ such that $q+\epsilon$ is in $A$ also, that is, $(q+\epsilon)^2<2$.
  ...
  \begin{align*}
    ...
    &=2
  \end{align*}
  We therefore conclude that $q+\epsilon$ is a rational number larger than $q$ and satisfying $(q+\epsilon)^2<2$. Thus $q$ is not the greatest element of $A$.
\end{proof}
...
We are now prepared to define the real number system $\R$ by ``filling in'' the gaps of $\Q$.

\begin{definition}
  The \emph{real number system} consists of the rational numbers $\Q$ together with, for every irrational cut $(A,\emptyset,B)$, a new point $p$ lying between the sets $A$ and $B$.
\end{definition}
...
\begin{example}
  ...
  \item The set $\set{q\in\Q\mid q<0\text{ or }q^2<2}$ has no maximum element. But it has upper bounds and its supremum is $\sqrt2$.
  \end{itemize}
\end{example}
