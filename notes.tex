...
For example, we define the ordering $<$ on real numbers as follows. If $\alpha_1$ and $\alpha_2$ are real numbers corresponding to cuts $(A_1,P_1,B_1)$ and $(A_2,P_2,b_2)$ are cuts, then we say $\alpha_1\leq\alpha_2$ if and only if $A_1\subseteq A_2$. It is straightforward to check this definition satisfies the usual axioms of $\leq$.
...
In the rational number system $\Q$, sets don't need to have a supremum. For example if $(A,\emptyset,B)$ is an irrational cut, then the set $A$ has no supremum in $\Q$. We now complete the proof that the cut at $\sqrt2$ is really a cut, and therefore its left set $A$ has no supremum in $\Q$. The proof turns out to be more technical than one might expect. It will provide us with a not-so-soft example of the kind of arguments we will see later in the course.

\begin{theorem}
  \label{thm:root-2-cut}
  Let $(A,\emptyset,B)$ be the triple from Equation~\ref{eq:irrationalcut}. Then $(A,\emptyset,B)$ is a cut in $\Q$, and $A$ has no supremum in $\Q$.
\end{theorem}

\begin{proof}
  The key step is to verify that $A$ has no greatest element. Let $q\in A$ be given; we will show $q$ is not the greatest element of $A$. It is clear that a negative number cannot be the greatest element of $A$, so we may assume that $q\geq0$ and $q^2<2$.
...
\begin{proof}[Proof sketch]
  By our definition of the real numbers, every $\alpha\in S$ corresponds to a cut $(A_\alpha,P_\alpha,B_\alpha)$. From these cuts we define a new cut cut $(A,P,B)$ by letting $A=\bigcup A_\alpha$ and $P$ and $B$ be determined from $A$. We leave it to the reader to verify that $(A,P,B)$ is really a cut. Let $\beta$ be the real number corresponding to the cut $(A,P,B)$.
...
For example, the rational number system $\Q$ is an ordered field that is not complete, since we checked that the set $A$ in the cut corresponding to $\sqrt2$ has no supremum in $\Q$. On the other hand we showed that the real number system $\R$ is an ordered field which is complete. The results of this discussion are summarized in the following big theorem.
...
As soon as one accepts this theorem, it is never necessary to speak of cuts again. In the future we can describe a real number in many ways: algebraically, as a limit, as a summation, or of course by its decimal expansion.
...
% 
%   The supremum of the set $A=\set{q\in\Q\mid q<0\text{ or }q^2<2}$ is $\sqrt2$.
%   
%   The number $\sqrt2$ is an upper bound of $A$, simply by its definition as a number which fills the gap in Equation~\eqref{eq:irrationalcut}. To see that it is the least upper bound, suppose that $\alpha<\sqrt2$ and $\alpha$ is also an upper bound of $A$. Then we can apply the argument of Lemma~\ref{root-2-cut} to find $\epsilon>0$ such that $\alpha+\epsilon<\sqrt2$. (In the lemma, $q$ was a rational number, but the algebra is the same for any real number $\alpha$.) Thus $\alpha$ is not an upper bound of $A$.
