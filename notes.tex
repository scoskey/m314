...
\begin{exerc}[Abbott, ex 4.3.1]
  Let $f(x)=\sqrt[3]{x}$.
  \begin{enumerate}
    \item Prove that $f(x)$ is continuous at $c=0$.
    \item Prove that $f(x)$ is continuous at $c\neq0$. [Hint: you will need the identity $a^3-b^3=(a-b)(a^2+ab+b^2)$.]
  \end{enumerate}
\end{exerc}
...
The development of differential calculus now consists of two pieces: using the definition to calculate a library of formulas for the derivatives of common functions, and using the definition to establish the algebraic laws of the derivative operation itself.
...
In the example above, the discontinuous derivative $f'(x)$ did not just have a simple jump discontinuity, but something much uglier. In our final result of the section, we will show that this ugliness can't be avoided. The result states that while derivatives need not be continuous, they still satisfy the \emph{intermediate value property}, that is, the conclusion of the intermediate value theorem. First we need the following familiar lemma.
