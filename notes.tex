...
We can think of a subset $A$ of some universe $X$ as dividing the universe into two parts: the points in $A$ and the points in $A^c$. However this divide is purely set-theoretic in that it doesn't take into account the relative placement of the points. If one the other hand we have some idea about the location or ordering of the points of $X$, then we can think of $A$ as dividing the universe into \emph{three} parts: the points well inside $A$, the points well outside of $A$, and the points on the boundary of $A$.

In order to define these concepts formally, first recall the neighborhood notation that we introduced alongside the definition of convergence. For any point $x$, the \emph{$\epsilon$-neighborhood of $x$} consists of all points that are less than $\epsilon$ away from $x$. In symbols:
\[V_\epsilon(x)=\set{y\in\RR\mid|y-x|<\epsilon}
\]

In other words, the neighborhood $V_\epsilon(x)$ is an interval centered at $x$, that is, $(x-\epsilon,x+\epsilon)$. Note however that if we were to make the analogous definition for $\RR^2$ we would define $V_\epsilon(x)=$ a disk centered at $x$, for $\RR^3$ we would define $V_\epsilon(x)=$ a ball centered at $x$, and so on.
...
Intuitively, if we think of points of $A$ as colored red and points of $A^c$ as colored blue, the boundary of $A$ consists of those points such that no matter how far you zoom in on them, you still see the colors red and blue.
...
Thus, to return to the motivation at the beginning of the section, any set $A$ divides $\RR$ into three disjoint pieces: the interior of $A$, the exterior of $A$, and the boundary of $A$.
...
\begin{proof}
  The first statement holds because every interior point of $A$ is an element of $A$, and the definiton of $A$ being open says precisely that every element of $A$ is an interior point of $A$.
...
\begin{exerc}
  Show that if $x\in A'$ then there exists a sequence $a_n$ such that $\{a_n\}\subset A$ and $\lim a_n=x$.
\end{exerc}

\begin{exerc}
  For any set $A$, show that $\inte(A)$ and $\ext(A)$ are open, and $\partial A$ and $\overline{A}$ are closed.
\end{exerc}
